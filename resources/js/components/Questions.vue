    ...
    <div class="col-lg-8 mx-auto" id="content">
        <div class="item" v-for="question in questions" :key="question.id">
            <div class="header" :style="{ direction:arDir(question.lg) }">
                <h1 @click="showQuestion(question.slug)"> {{question.title}} </h1>
                <div class="head-right">
                    <div @click="showQuestion(question.slug)" :class="{'mx-3' : marginLeft(question.lg)}">
                        <p class="poster">
                        {{ question.username }}
                        </p>
                        <p class="time">
            ...
            <div class="reponds">
            <h1 @click="afficheDisscusions=!afficheDisscusions">Discussion  <span> {{ question.solutions.length }} </span> </h1>
            <div class="form mb-4">
                <form v-on:submit.prevent="formSubmit(question.id)"  enctype="multipart/form-data" method="POST">
